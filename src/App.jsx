...
export default function App() {
  ...
  const handleGoogleLogin = async () => {
    if (!auth) {
      ...
      showToast('Erro de configuração do Firebase');
      return;
    }
  ...
  // Tela de login - CORRIGIDA: verificação simplificada
  if (!user) {
    return (
      <div className="min-h-[100dvh] w-full flex flex-col items-center justify-center bg-black text-white">
        <div className="flex flex-col items-center gap-6 p-8 rounded-2xl bg-white/5 border border-white/10 max-w-sm w-full mx-4">
          <div className="flex flex-col items-center gap-2">
            ...
            <h1 className="text-2xl font-bold tracking-tight">RhythmSecret</h1>
            <p className="text-sm text-gray-400 text-center">Faça login para acessar o app</p>
          </div>
          <button
            onClick={handleGoogleLogin}
            disabled={isLoggingIn}
            className="flex items-center gap-3 w-full justify-center px-6 py-3 bg-white text-gray-900 font-semibold rounded-xl hover:bg-gray-100 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoggingIn ? (
              <>Carregando...</>
            ) : (

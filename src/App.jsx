...
// Agrupamento para exibição no modal
...
export default function App() {
  ...
  const [showShortcutsModal, setShowShortcutsModal] = useState(false); // Novo estado Modal Atalhos
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      // Ignorar digitação em campos (como o input targetBpm)
      if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') return;

      const key = e.key.toLowerCase();

      // Prevenção de default para setas e espaço
      ...
      // --- PRO FEATURES ---
        ...
        if (!isUiLocked && appMode === 'PRO') { setIsBpmProgressEnabled(prev => !prev); showToast(`⚡ Auto BPM alternado`); }
      } 
      // --- VOLUME MIXER ---
